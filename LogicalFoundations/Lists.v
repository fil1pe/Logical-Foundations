...
Fixpoint aux_oddmembers (n:nat) : natlist :=
  match n with
  | 0 => nil
  | S 0 => [S 0]
  | S n' => match aux_oddmembers(n') with
            | nil => [S n']
            | n'' => nil
            end
  end.
...
Fixpoint oddmembers (l:natlist) : natlist :=
  match l with
  | nil => nil
  | n :: t => aux_oddmembers(n) ++ oddmembers (t)
  end.
...
Fixpoint aux_count (v:nat) (n:nat) : nat :=
  match (n =? v) with
  | false => 0
  | true => 1
  end.
...
Fixpoint subset (s1:bag) (s2:bag) : bool :=
  match s1 with
  | nil => true
  | n :: t => (0 <? count n s2) && subset t (remove_one n s2)
  end.
...
Proof.
  ...
  induction l.
  - reflexivity.
  - rewrite IHl. simpl. replace (n =? n) with (true). reflexivity.
